"""Legacy keyboard controller (translation only) for troubleshooting.

Controls
- Translation: 1/2 → ±X, 3/4 → ±Y, 5/6 → ±Z

Notes
- Uses carb key enums directly (digits only) to match older behavior.
- No rotation output; combine with your existing apply_palm_motion call.
"""
...
class KeyboardControllerLegacy:
    """Translate numerical key presses into Cartesian wrist motions (legacy)."""
    ...
    def __init__(self, step_size: float = 0.2) -> None:
        ...
        try:
            ...
            self._active = True
            print("[KeyboardControllerLegacy] Controls: 1/2→±X, 3/4→±Y, 5/6→±Z (CTRL+C to exit).")
        except Exception as exc:  # pragma: no cover
            print(f"[WARN][KeyboardControllerLegacy] Input interface unavailable ({exc}). Keyboard control disabled.")
    ...
    def rotation(self) -> tuple[float, float, float]:
        """Legacy controller has no rotation; keep API-compatible."""
        return 0.0, 0.0, 0.0
...
__all__ = ["KeyboardControllerLegacy"]
