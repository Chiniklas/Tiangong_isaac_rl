"""Placeholder reward terms for the UniGraspTransformer task.

These return zero so the RewardManager wiring works; replace with
task-specific logic later.
"""
...
# these are implicit rewards listed in the paper
def grasp_reward_rd(env) -> torch.Tensor:
...
#==============================================================
# TODO: check each reward term to make sure they match the original unigrasptransformer
# these are explicit reward terms listed in the upstream unigrasptransformer
...
def grasp_hold_flag(env) -> torch.Tensor:
    # Indicator that fingers/hand are within contact thresholds (e.g., Chamfer distance < lambda_c) to switch phases.
    ...
    fingertip_pos = body_pos[:, fingertip_ids, :] if len(fingertip_ids) > 0 else palm_pos.unsqueeze(1)

    # mean fingertip distance to object center
    mean_tip_dist = torch.norm(fingertip_pos - object_pos.unsqueeze(1), dim=-1).mean(dim=1)
    hold = mean_tip_dist < CONTACT_DIST_THRESHOLD

    # also require palm within a looser threshold
    hold &= torch.norm(palm_pos - object_pos, dim=-1) < (CONTACT_DIST_THRESHOLD * 2.0)
    return hold.float()
...
def grasp_delta_init_qpos(env) -> torch.Tensor:
    ...
    return torch.norm(dof_pos - target, p=2, dim=1)
...
def grasp_right_hand_exploration_dist(env) -> torch.Tensor:
    ...
    return torch.norm(palm_pos - sample, dim=-1)
...
def grasp_right_hand_body_dist(env) -> torch.Tensor:
    ...
    return dists.mean(dim=1) if masked_pos.numel() > 0 else torch.zeros(env.num_envs, device=env.device)
...
def grasp_right_hand_joint_dist(env) -> torch.Tensor:
    ...
    # penalize magnitude as a proxy
    return torch.norm(joint_pos, dim=1)
...
def grasp_goal_dist(env) -> torch.Tensor:
    ...
    return torch.norm(object_pos - goal, dim=-1)
...
def grasp_goal_rew(env) -> torch.Tensor:
    ...
    return torch.clamp(0.9 - 2.0 * dist, min=0.0)
...
def grasp_bonus(env) -> torch.Tensor:
    ...
    dist = grasp_goal_dist(env)
    return torch.where(dist <= GOAL_DIST_THRESHOLD, 1.0 / (1.0 + 10.0 * dist), torch.zeros_like(dist))
...
def grasp_right_hand_pose(env) -> torch.Tensor:
    ...
    dof_pos = env.robot.data.joint_pos
    target = torch.zeros_like(dof_pos)
    return torch.norm(dof_pos - target, p=1, dim=1)
